...
JOBS = {
    ...
    'gate1':  dict(split='val', n_val=400, models=['y1_full'], stats_variant='V1_expect',
                   variants=[V('argmax', 'argmax'),
                             # literal spec: drop STOP, renormalise over ids 0..63
                             V('V1_expect', 'expect'),
                             V('V2_expect_topk5', 'expect_topk', topk=5),
                             V('V3_expect_T0.5', 'expect_temp', temperature=0.5),
                             # STOP-aware: support {0..63} U {128}, STOP -> value 0
                             V('V1s_expect_stop', 'expect', stop_aware=True),
                             V('V2s_topk5_stop', 'expect_topk', topk=5, stop_aware=True),
                             V('V3s_T0.5_stop', 'expect_temp', temperature=0.5, stop_aware=True)]),
...
def main():
    ...
    for i in my:
        ...
        cv[i] = errs(cv_pred(v0, yaw0), gt_local(t)); curv[i] = maxcurv(t)
    ...
    for mname in job['models']:
        ...
        sacc = [{'entropy': 0., 'top1': 0., 'dead_mass': 0., 'stop_mass': 0.} for _ in range(24)]
        sn = 0
    ...
    payload = {'per': per, 'cv': cv, 'curv': curv, 'slot_stats': slot_stats}
    ...
    for g in G:
        ...
        CV.update(g['cv']); CURV.update(g['curv'])
...
def report(res):
    ...
    for sname, sub in subsets:
        if not sub: continue
        print(f"\n-- {sname} (n={len(sub)}) --")
        print(f"{'model/variant':28} " + " ".join(f"{'ADE'+l:>8}" for l in labs) +
              f" {'medADE6s':>9} {'FDE6s':>8}")
        row = [np.mean([cvd[i]['ade'][l] for i in sub]) for l in labs]
        med = np.median([cvd[i]['ade']['6s'] for i in sub])
        f6 = np.mean([cvd[i]['fde']['6s'] for i in sub])
        print(f"{'CV (baseline)':28} " + " ".join(f"{v:8.3f}" for v in row) + f" {med:9.3f} {f6:8.3f}")
        for m in res['per']:
            for vn in res['per'][m]:
                ...
                for k in ks:
                    ...
                    f6v = np.mean([min(r['fde']['6s'] for r in P[i][:k]) for i in sub if i in P])
                    tag = f"{m}/{vn}" + (f"@{k}" if K > 1 else "")
                    print(f"{tag:28} " + " ".join(f"{np.mean(a[l]):8.3f}" for l in labs) +
                          f" {np.median(a['6s']):9.3f} {f6v:8.3f}")
    for m, s in res.get('slot_stats', {}).items():
        print(f"\n-- per-slot decode stats [{m}] (n={s['n']}) --")
        print(f"{'slot':6} {'kind':6} {'entropy':>8} {'top1':>7} {'dead':>7} {'stop':>7}")
        for i, d in enumerate(s['mean']):
            print(f"{i:<6} {'accel' if i<12 else 'curv':6} {d['entropy']:8.3f} {d['top1']:7.3f} "
                  f"{d['dead_mass']:7.4f} {d['stop_mass']:7.4f}")
